...
async def staff_show_banks_account(  # Доделать после проработки проверки!!!!!
    ...
    params = {"owner": changer_id, "isDeleted": False}
    response: dict = await api_gateway.get(
        path=r.changerRoutes.banksCheker, params=params, exp_code=[200]
    )
        ...
        if banks_list:
            ...
            for bank in banks_list:
                self_msg = await bot.send_message(
                    call.from_user.id,
                    text=await msg_maker.staff_show_editable_banks(bank),
...
async def staff_bank_account_setter(
    ...
    key = (
        "isActive"
        if action in ["staff_edit_banks_active", "staff_edit_banks_inactive"]
        else "isDeleted"
    )

    value = (
        True
        if action in ["staff_edit_banks_active", "staff_edit_banks_delete"]
        else False
    )
    ...
    if not exception:
        ...
        await staff_show_banks_account(  # Доделать после проработки проверки!!!!!
            call, state, callback_data, api_gateway, bot
        )
    else:
...
async def setup_edit_banks_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(
        ...
        StaffOfficeData.filter(F.action == "staff_show_accounts"),
    )
    dp.callback_query.register(
