...
from core.keyboards import changer_kb, admin_kb
...
async def staff_show_transfers(
    call: CallbackQuery, state: FSMContext, callback_data: StaffEditData, bot: Bot
):
    """ """
    ...
    if callback_data.action == "staff_transfers_get_detail":
        ...
        if transfer_detail["userProofType"] == "photo":
            mainMsg = await bot.send_photo(
                ...
                caption=await msg_maker.staff_show_uncompleted_transfer_detail(
                    transfer_detail
                ),
        ...
        if not exception:
            ...
            await bot.send_message(
                ...
                reply_markup=await admin_kb.get_claim_contacts_toadmin(
                    transfer_detail["user"], call.from_user.id
                ),
            )
            if transfer_detail["userProofType"] == "photo":
...
async def staff_transfer_proof_getter(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        # after 3 seconds
        del_msg: Message = await bot.send_message(text=msg.type_error_message)
        ...
        del_msg = await bot.send_message(
            message.from_user.id, text=msg.staff_get_proof_success
        )
        await asyncio.sleep(3)
        try:
            await del_msg.delete()
...
async def setup_exchange_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(
        ...
        StaffOfficeData.filter(
            F.action.in_({"staff_show_transfers", "changer_missed_events"})
        ),
    )
    dp.callback_query.register(
