...
async def stuff_create_new_offer_currency(
    ...
    """
    if callback_data.action in ["new_buy_offer_currency", "new_sell_offer_currency"]:
        ...
        await call.message.edit_text(
            text=msg.chr_enter_rate, reply_markup=await changer_kb.sfuff_cancel_button()
        )
...
async def stuff_create_new_offer_rate(message: Message, state: FSMContext):
    ...
    try:
        ...
        await mainMsg.edit_text(
            text=msg.type_error_msg, reply_markup=await changer_kb.sfuff_cancel_button()
        )
    else:
        # if all ok
        ...
        if callback_data.action == "create_new_offer_banks":
            ...
            if banks:
                ...
                await call.message.edit_text(
                    ...
                    reply_markup=await changer_kb.stuff_new_offer_banks_w_next(banks),
                )
            else:
...
async def stuff_create_new_offer_final(
    ...
    # if all offer values are correctly - he allow post offer
        ...
        response: dict = await api_gateway.post(
            path=r.changerRoutes.myOffers, data=post_data, exp_code=[201]
        )
...
async def setup_create_offer_handlers(dp: Dispatcher):
    ...
    dp.message.register(stuff_create_new_offer_rate, FSMSteps.STUFF_OFFERS_RATE, F.text)
    dp.message.register(
        stuff_create_new_offer_min_amount, FSMSteps.STAFF_OFFERS_MINAMOUNT, F.text
    )
    dp.message.register(
    ...
    dp.message.register(
        stuff_create_new_offer_name_setter, FSMSteps.STAFF_OFFERS_SETNAME, F.text
    )
    ...
    dp.callback_query.register(
        ...
        StaffEditData.filter(F.action.in_({"set_min&max_amount"})),
    )
    dp.callback_query.register(
        stuff_create_new_offer_max_amount,
        StaffEditData.filter(
            F.action.in_({"staff_set_max_amount", "staff_pass_min_amount"})
        ),
    )
    dp.callback_query.register(
