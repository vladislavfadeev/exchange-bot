...
async def timeout_claims_handler(call: CallbackQuery, state: FSMContext):
    """ """
...
async def setup_claims_handlers(dp: Dispatcher):
    ...
    dp.message.register(get_user_timeout_claims_proof, FSMSteps.USER_TIME_EXPIRED_PROOF)
    dp.callback_query.register(
        timeout_claims_handler,
        UserExchangeData.filter(F.action == "send_money_on_expired_offer"),
    )
