...
from core.utils.notifier import alert_message_sender, transfers_getter_changer
import asyncio


async def command_start(
    message: Message, state: FSMContext, bot: Bot, api_gateway: SimpleAPI
):
    """
...
async def command_login(
    ...
    if not isStuff:
        ...
        if not exception:
            if code == 404:
                ...
                await message.delete()
                info_msg = await message.answer(text=msg.staff_404)
    ...
    # if bot state have information about current user
        ...
        await message.delete()
        del_msg = await bot.send_message(message.from_user.id, text=msg.staff_islogin)
...
async def command_logout(
    ...
    # if user have not staff status
        ...
        await message.delete()
        del_msg = await bot.send_message(message.from_user.id, text=msg.user_not_loggin)
        await asyncio.sleep(2)
        try:
            await del_msg.delete()
        except:
...
async def get_help(
    ...
    await call.message.edit_text(
        text=msg.help_message, reply_markup=await home_kb.user_back_home_inline_button()
    )
...
async def del_not_handled_message(message: Message, bot: Bot):
    ...
    del_msg = await bot.send_message(
        message.from_user.id, text=f'Unknown command: "{bill}"'
    )
    await asyncio.sleep(3)
    try:
        await del_msg.delete()
