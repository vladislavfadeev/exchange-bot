...
async def set_amount_msg_maker(offerData):
    ...
    message = (
        f'👌 Ваш выбор принят!\n\n'
        '⚡⚡⚡<b>Внимание!</b>⚡⚡⚡\n'
        'Дорогие пользователи нашего сервиса, '
        'ввиду высокой волатильности на валютном рынке '
        'обменники могут корректировать курс валют до нескольких раз '
        'в день. По этому, во избежание спорных ситуаций '
        'мы ограничили время создания заявки на обмен до 20 минут.\n\n'
        f'👇 Введите сумму {offerData["currency"]} для обмена: 👇'
        )
...
async def complete_set_new_bank(allData, api_gateway: SimpleAPI):
    ...
    if not exception:
        ...
        message = f'👌 Ваши реквизиты приняты! 👌\
                    ...
                    \n\n🏦 Банк: {acc["name"]}\
                    \n💳 Счет: <code>{acc["bankAccount"]}</code>\
                    \n\n⚠️ Обязательно! ⚠️\
...
async def final_step_timeout_message():
    ...
    message = (
        'Мы сожалеем но вы ебучий тормоз..\n'
        'Пока вы думали, все к хуям поменялось '
        'и теперь нужно оформлять заявку заново.'
        'Если вдруг вы умудрились перевести деньги '
        'жмите кнопку связаться с админом и он как '
        'батя все разрулит. Ну а хули.'
    )
    return message
