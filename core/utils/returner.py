...
async def main_msg_returner(bot: Bot, dp: Dispatcher, api_gateway: SimpleAPI):
    """
    Function execute every 30 seconds. It compares
    current time and last action time of every user and
    if it more than 2 minutes - function reset current
    user state, messages an return him to main message
    for he can receive notification.
    """
    # try to get all changers id from backend
    response: dict = await api_gateway.get(
        path=r.changerRoutes.changerIdList, exp_code=[200]
    )
    ...
    if not exception:
        ...
        for changer in id_list:
            state: FSMContext = FSMContext(
                bot=bot,
                storage=dp.storage,
                key=StorageKey(chat_id=changer, user_id=changer, bot_id=bot.id),
            )
            ...
            if current_state != home_state and time_delta.total_seconds() >= 120:
...
async def user_exchange_returner(bot: Bot, dp: Dispatcher, api_gateway: SimpleAPI):
    """
    ...
    response: dict = await api_gateway.get(path=r.homeRoutes.userInit, exp_code=[200])
    exception: bool = response.get("exception")
    if not exception:
        ...
        for user in user_list:
            state: FSMContext = FSMContext(
                bot=bot,
                storage=dp.storage,
                key=StorageKey(chat_id=user, user_id=user, bot_id=bot.id),
            )
                ...
                if delta.total_seconds() > 1200:
                    ...
                    if not exception:
                        ...
                        edit_time: datetime = datetime.strptime(
                            response_data.get("edited"), "%Y-%m-%dT%H:%M:%S.%f%z"
                        )
                        # if it has been changed or changer is not online
                        if edit_time > user_start_change_time or not changer_online:
                            mainMsg: Message = data.get("mainMsg")
